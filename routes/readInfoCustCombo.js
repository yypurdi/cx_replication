...
router.get('/', sessionChecker, function(req, res, next) {
    ...
    jsonObj.rowid = 0;
    req.session.previouspath = 'readInfoCustCombo?'+req._parsedUrl.query;
    parameter = {
        session: req.session,
        cookies: req.cookies,
    ...
    var readInfoCustCombo = new Promise((resolve, reject) => {
        setTimeout(() => {
            dao.readInfoCustCombo(jsonObj,function(err,rows) {
                if(err) res.status(err.code).end();
                else{
                    if(rows[0]===undefined) res.redirect('/searchInfoCustCombo');
                    else{ 
                        jsonObj = rows[0];
                        resolve(jsonObj);
                    }
    ...
    var sendResult = (results) => {
        ...
        res.render('InfoCustComboReadPage',parameter);
    }    
    Promise.all([readInfoCustCombo]).then(sendResult);
});
