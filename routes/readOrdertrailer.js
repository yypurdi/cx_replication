...
router.get('/', sessionChecker, function(req, res, next) {
    ...
    jsonObj.rowid = 0;
    req.session.previouspath = 'readOrdertrailer?'+req._parsedUrl.query;
    parameter = {
        session: req.session,
        cookies: req.cookies,
    ...
    var readOrdertrailer = new Promise((resolve, reject) => {
        setTimeout(() => {
            dao.readOrdertrailer(jsonObj,function(err,rows) {
                if(err) res.status(err.code).end();
                else{
                    if(rows[0]===undefined) res.redirect('/searchOrdertrailer');
                    else{ 
                        jsonObj = rows[0];
                        resolve(jsonObj);
                    }
    ...
    var sendResult = (results) => {
        ...
        res.render('OrdertrailerReadPage',parameter);
    }    
    Promise.all([readOrdertrailer]).then(sendResult);
});
