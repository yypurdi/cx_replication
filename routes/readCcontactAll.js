...
router.get('/', sessionChecker, function(req, res, next) {
    ...
    jsonObj.rowid = 0;
    req.session.previouspath = 'readCcontactAll?'+req._parsedUrl.query;
    parameter = {
        session: req.session,
        cookies: req.cookies,
        ...
        setTimeout(() => {
            dao.readCcontactAll(jsonObj,function(err,rows) {
                if(err) res.status(err.code).end();
                    ...
                    if(rows[0]===undefined) res.redirect('/searchCcontactAll');
                    else{ 
                        jsonObj = rows[0];
                        resolve(jsonObj);
                    }
    ...
    var sendResult = (results) => {
        ...
        res.render('CcontactAllReadPage',parameter);
    }    
